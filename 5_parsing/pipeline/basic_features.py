...
class BasicHandler(Handler):
    ...
    def _parse_salary_and_currency(self, series: pd.Series) -> tuple[pd.Series, pd.Series]:
        ...
        # хвост после числа
        tail = s.str.extract(r"\d+\s*(.*)", expand=False).fillna("").str.strip()
        tail_lower = tail.str.lower()

        def detect_currency(t: str) -> str:
            if not t:
                return "unknown"
            if "руб" in t or " rub" in t:
                return "rub"
            if "usd" in t or "$" in t:
                return "usd"
            if "eur" in t or "€" in t:
            ...
            if "kzt" in t:
                return "kzt"
            # можно расширить при желании
            return "unknown"

        currency = tail_lower.apply(detect_currency).astype("string")
    ...
    def process(self, context: dict) -> dict:
        ...
        df["has_car"] = has_car
        # sex тоже посчитал, но демо-обработчик ниже его перезапишет/подтвердит
        df["sex"] = sex
